<template>
  <div class="book-importer">
    <!-- 导入方式选择 -->
    <div class="import-methods">
      <BaseCard class="method-card">
...
// Emits
const emit = defineEmits(['import-complete', 'import-start', 'import-progress'])
...
const importSingleFile = async () => {
  try {
    ...
    if (filePath) {
      await processImport([filePath], '导入单个文件')
    }
  } catch (error) {
...
const importFolder = async () => {
  try {
    ...
    if (folderPath) {
      ...
      if (files.length > 0) {
        await showImportPreview(files)
      } else {
        showToast('文件夹中没有找到支持的电子书文件', 'warning')
      }
    }
  } catch (error) {
...
const handleDrop = async (event) => {
  ...
  try {
      ...
      if (item.kind === 'file') {
        ...
        if (entry) {
          if (entry.isFile) {
            ...
            const folderFiles = await scanFolder(entry.fullPath)
    ...
    showToast('拖拽导入失败: ' + error.message, 'error')
  }
}
...
const processImport = async (filePaths, title) => {
  ...
  emit('import-start', { files: filePaths })
  ...
  try {
    ...
    if (successCount > 0) {
      showToast(`成功导入 ${successCount} 本图书${errorCount > 0 ? `，${errorCount} 个失败` : ''}`, 'success')
