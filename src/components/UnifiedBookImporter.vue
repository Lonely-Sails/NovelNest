<template>
    <div class="unified-importer">
        <!-- 导入区域 -->
        <div class="import-zone" :class="{ 'drop-active': isDragOver }" @drop="handleDrop" @dragover="handleDragOver"
            ...
            <div class="import-content">
                ...
                <div class="import-actions">
                    <BaseButton @click="importSingleFile" variant="primary" :loading="importing.single" icon="📖">
                        选择文件
                    </BaseButton>

                    <BaseButton @click="importFolder" variant="secondary" :loading="importing.folder" icon="📁">
                        选择文件夹
                    </BaseButton>

                    <BaseButton @click="showSettings = true" variant="outline" size="small" icon="⚙️"
                        class="settings-btn">
                        设置
                    </BaseButton>
                </div>
        ...
        <div v-if="importProgress.show" class="import-progress">
            <BaseCard>
                <div class="progress-header">
                    ...
                    <BaseButton v-if="importProgress.canCancel" @click="cancelImport" variant="outline" size="small">
                        取消
                    </BaseButton>
                </div>
                ...
                <div v-if="importProgress.results.length > 0" class="import-results">
                    ...
                    <div class="results-list">
                        <div v-for="result in importProgress.results" :key="result.file" class="result-item"
                            :class="result.status">
        ...
        <!-- 导入预览 -->
        <Modal v-if="showPreview" @close="closePreview" title="导入预览" size="large">
            <div class="import-preview">
                ...
                <div class="preview-filters" v-if="showFilters">
                    <div class="filter-group">
                        ...
                        <div class="format-checkboxes">
                            <label v-for="format in availableFormats" :key="format" class="checkbox-label">
                ...
                <div class="preview-list">
                    <div v-for="file in filteredPreviewFiles" :key="file.path" class="preview-item"
                ...
                <div class="preview-summary">
                    <p>
                        ...
                        <span v-if="newFiles.length > 0">（{{ newFiles.length }} 个新文件）</span>
                        <span v-if="existingFiles.length > 0">（{{ existingFiles.length }} 个已存在）</span>
                    </p>
                </div>
            </div>
            ...
            <template #actions>
                ...
                <BaseButton @click="confirmImport" variant="primary" :disabled="selectedFiles.length === 0">
        ...
        <!-- 导入设置 -->
        <Modal v-if="showSettings" @close="closeSettings" title="导入设置" size="medium">
            <div class="import-settings">
                <div class="setting-group">
                    <label class="setting-label">支持的文件格式</label>
                    <div class="format-list">
                        <label v-for="format in supportedFormats" :key="format.ext" class="format-item">
                ...
                <div class="setting-group">
                    ...
                    <div class="option-list">
                        ...
                        <label class="option-item">
                            <input type="checkbox" v-model="importSettings.autoExtractMetadata">
                            自动提取元数据
                        </label>
                        <label class="option-item">
            ...
            <template #actions>
                <BaseButton @click="resetSettings" variant="outline">
                    重置
                </BaseButton>
...
const filteredPreviewFiles = computed(() => {
    ...
    return previewFiles.value.filter(f => selectedFormats.value.includes(f.format))
})
...
const allSelected = computed(() => {
    return filteredPreviewFiles.value.length > 0 &&
...
const processImport = async (filePaths, title) => {
    ...
    try {
        if (filePaths.length === 1) {
            ...
            try {
                ...
                for (let i = 0; i < filePaths.length; i++) {
                    ...
                    importProgress.value.currentFile = fileName
                    importProgress.value.percentage = Math.round(((i + 1) / filePaths.length) * 100)
        ...
        const successCount = importProgress.value.results.filter(r => r.status === 'success').length
        const errorCount = importProgress.value.results.filter(r => r.status === 'error').length
        const skippedCount = importProgress.value.results.filter(r => r.status === 'skipped').length
...
const saveSettings = () => {
    ...
    localStorage.setItem('supported_formats', JSON.stringify(supportedFormats.value))
    showSuccess('设置已保存')
    closeSettings()
}
...
onMounted(() => {
    try {
        ...
        if (savedSettings) {
            importSettings.value = { ...importSettings.value, ...JSON.parse(savedSettings) }
        }
